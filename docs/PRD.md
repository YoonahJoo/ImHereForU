...
- 클릭, 더블클릭, 롱프레스, 드래그, daily/focus 상태에 따른 캐릭터 반응
...
기능 검증 단계. 핵심 인터랙션 시스템이 동작하는지 확인하는 것이 목표였다.

- 이모지 기반 캐릭터 표정 시스템 (10종)
- 클릭 / 더블클릭 / 드래그 / 롱프레스 / 방치 감지 인터랙션
- Daily / Focus 모드 전환 + 25분 포모도로 타이머
- 선물 시스템 + Gift Room + Settings

---

### v2.0 — 디자인 정의 (진행 중, 2026-06 ~)

감성 전달 단계. 기능이 동작하는 것을 넘어, 앱의 핵심 가치인 Emotional Presence가 시각적으로 전달되는지 검증한다.

- 책 세계관 UI — 투명 창 + 책 PNG 배경 + 책 열리는 인트로 애니메이션
- 실제 캐릭터 일러스트 적용 (yoonah.png)
- 캐릭터 레이어 애니메이션 시스템 — 눈깜빡임, 손흔들기, 표정 전환
- 인터랙티브 에셋 오버레이 (sunflower, bouquet, memo 등)
- Settings / Gift Room 책 UI 통합

---

### v3.0 — 캐릭터 독립 (예정)

최종 비전 단계. 캐릭터가 책이라는 공간에서 나와 사용자의 데스크탑 위에서 독립적으로 존재한다.

- 캐릭터가 책 밖으로 나오는 애니메이션
- Electron 창을 데스크탑 전체로 확장, 캐릭터가 화면 어디서든 자유롭게 이동
- v2.0에서 구축한 표정 / 애니메이션 시스템 그대로 재사용
- 데스크탑 위에서의 새로운 인터랙션 패턴 설계